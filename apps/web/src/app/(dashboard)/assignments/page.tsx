          ...
          <table className="w-full text-sm" role="grid" aria-label="Assignments list">
            ...
            <tbody>
              {isLoading
                ...
                : data?.data.length === 0
                  ? <tr><td colSpan={8} className="px-4 py-12 text-center text-text-500">No assignments found</td></tr>
                  : data?.data.map((a) => (
                    <tr key={a.id} className="border-b border-surface-100 hover:bg-surface-50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <Link href={`/assignments/${a.id}`} className="text-xs text-spira-700 hover:underline">View</Link>
                      </td>
                    </tr>
                  ))}
            </tbody>
